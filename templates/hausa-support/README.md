# Hausa customer-support chatbot

A small Next.js app. Customers type in Hausa (or English) and N-ATLaS replies in Hausa. The API key never reaches the browser.

## Setup

```bash
cd templates/hausa-support
npm install
...
Set the root directory to `templates/hausa-support` if the form does not fill it in. Add the two env vars, then deploy.
